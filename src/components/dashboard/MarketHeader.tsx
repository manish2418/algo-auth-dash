...
import { TrendingUp, Home, ShoppingCart, PieChart, DollarSign, ChevronDown } from "lucide-react";
 ...
 */
export const MarketHeader = ({ marketIndices, onLogout }: MarketHeaderProps) => {
  return (
            ...
            {marketIndices.map((index) => (
              <div key={index.name} className="flex items-center gap-2">
                ...
                <div className="flex items-center gap-1">
                  ...
                  {!index.isPositive && <ChevronDown className="h-3 w-3 text-red-400" />}
                </div>
              </div>
            ))}
          </div>
